...
export class VariableHandler extends AbstractHandler {
    ...
    protected toPlantUML(...variables: IPlantUMLComposite[]): string[] {
        ...
        variables.forEach((variable: IPlantUMLComposite): void => {
            ...
            const variablePlantUMLObjectDeclaration =
                `class ${variablePlantUMLObjectName}<${variable.type}> << (V, green) >>`;
            ...
            document.push(...variable.variables.map(
                (subVariable: IPlantUMLComposite): string => `${INDENTATION}${subVariable.name}`));
            document.push("}");
        });
        return document;
    }
